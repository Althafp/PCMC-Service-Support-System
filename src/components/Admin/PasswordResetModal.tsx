...
export function PasswordResetModal({ isOpen, onClose, onSuccess, user }: PasswordResetModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { data: functionResult, error: functionError } = await supabase.functions.invoke('reset-password', {
        body: {
          userId: user.id,
      ...
      if (functionError) {
        ...
        throw new Error(`Failed to reset password: ${functionError.message}`);
      }

      if (!functionResult || !functionResult.success) {
        throw new Error(functionResult?.error || 'Failed to reset password - no response from server');
      }
