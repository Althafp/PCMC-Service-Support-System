...
-- drop table "storage"."iceberg_tables";
  ...
  create policy "Allow Viewing 1ffg0oo_0"
  on "storage"."objects"
  as permissive
  for select
  to authenticated
using (((bucket_id = 'images'::text) AND (auth.role() = 'authenticated'::text)));
  ...
  create policy "Allow authenticated uploads 1ffg0oo_0"
  on "storage"."objects"
  as permissive
  for insert
  to authenticated
with check (((bucket_id = 'images'::text) AND (auth.role() = 'authenticated'::text)));
  ...
  create policy "Allow user deletes 1ffg0oo_0"
  on "storage"."objects"
  as permissive
  for delete
  to authenticated
using (((bucket_id = 'images'::text) AND (auth.role() = 'authenticated'::text)));
  ...
  create policy "Allow user updates 1ffg0oo_0"
  on "storage"."objects"
  as permissive
  for update
  to authenticated
